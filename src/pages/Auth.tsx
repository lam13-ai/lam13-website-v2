    ...
    try {
      if (mode == "signin") {
        ...
        if (!data.success) {
          ...
          localStorage.setItem("lam13_user", JSON.stringify({
  ...
  return <div className="min-h-screen flex bg-background">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-primary via-primary/90 to-accent/80">
        ...
        <div className="absolute inset-0 opacity-20">
          ...
          <div className="absolute bottom-1/4 -right-20 w-96 h-96 rounded-full bg-white/10 blur-3xl animate-pulse" style={{
          animationDelay: "1s"
        }} />
        </div>
        ...
        <div className="relative z-10 flex flex-col justify-start p-12 pt-16 text-white">
          ...
          <div className="space-y-4">
            {["Unlimited conversations with our AI agents", "Save and organize your strategic insights", "Access exclusive governance frameworks"].map((feature, i) => <div key={i} className="flex items-center gap-3">
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex bg-secondary/30 rounded-xl p-1 mb-8">
            <button onClick={() => setMode("signin")} className={`flex-1 py-3 px-4 rounded-lg text-sm font-medium transition-all ${mode === "signin" ? "bg-background text-foreground shadow-md" : "text-muted-foreground hover:text-foreground"}`}>
              Sign In
            </button>

            <button onClick={() => setMode("register")} className={`flex-1 py-3 px-4 rounded-lg text-sm font-medium transition-all ${mode === "register" ? "bg-background text-foreground shadow-md" : "text-muted-foreground hover:text-foreground"}`}>
              Create Account
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Validation Errors */}
            {validationErrors.length > 0 && <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-3">
                {validationErrors.map((error, i) => <p key={i} className="text-destructive text-sm">{error}</p>)}
              </div>}
            {errorRegister && mode === "register" && <div className="text-destructive text-sm text-center mb-2">{errorRegister}</div>}
            ...
            <div className="space-y-2">
              ...
              {errorSignIn && mode === "signin" && <div className="text-red-500 text-sm text-center mb-2">{errorSignIn}</div>}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? "text" : "password"} placeholder={mode === "signin" ? "Enter your password" : "Create a password"} value={formData.password} onChange={e => setFormData({
                ...formData,
              ...
              {mode === "register" && formData.password && <div className="mt-2 space-y-1">
                  {[{
                key: 'minLength',
                ...
                return <div key={key} className={`flex items-center gap-2 text-xs ${isValid ? 'text-green-600' : 'text-muted-foreground'}`}>
            ...
            {mode === "register" && <div className="space-y-2">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                  <Input id="confirmPassword" type={showPassword ? "text" : "password"} placeholder="Confirm your password" value={formData.confirmPassword} onChange={e => setFormData({
                ...formData,
            ...
            <Button type="submit" className="w-full h-12 bg-accent hover:bg-accent/90 text-accent-foreground font-semibold group">
          ...
          {/* Terms */}
          {mode === "register" && <p className="text-xs text-muted-foreground text-center mt-6">
