...
const TryUs = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      try {
        ...
        assistantContent = data.choices?.[0]?.message?.content || data.message?.content || data.content || data.response || responseText;
      } catch {
  ...
  return <div className="h-[100dvh] flex bg-background relative overflow-hidden">
      {/* Mobile Overlay */}
      {sidebarOpen && isMobile && <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={() => setSidebarOpen(false)} />}
      
      {/* Sidebar */}
      <aside className={`
          ${isMobile ? `fixed w-[280px] h-full z-50 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}` : `relative ${sidebarOpen ? "w-72" : "w-0"}`}
        ...
        <div className="p-4 border-b border-border/50 flex items-center justify-between min-w-[280px] md:min-w-[288px]">
          ...
          <Button variant="ghost" size="icon" className="md:hidden text-foreground/70" onClick={() => setSidebarOpen(false)}>
            <X className="w-5 h-5" />
          </Button>
        </div>
        
        <div className="p-4 min-w-[280px] md:min-w-[288px]">
          <Button onClick={handleNewChat} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground gap-2">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2 min-w-[280px] md:min-w-[288px]">
          ...
          {chats.map(chat => <button key={chat.id} onClick={() => handleSelectChat(chat.id)} className={`w-full text-left px-3 py-2 rounded-lg transition-colors flex items-center gap-2 text-sm ${activeChat === chat.id ? "bg-accent/20 text-accent" : "hover:bg-secondary/50 text-foreground/70"}`}>
        ...
        <div className="p-4 border-t border-border/50 min-w-[280px] md:min-w-[288px]">
          {loggedInUser ? <div className="space-y-3">
            ...
            setActiveChat("1");
              ...
              <Link to="/auth">
                <Button variant="outline" className="w-full gap-2 border-accent/50 text-accent hover:bg-accent hover:text-accent-foreground">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-14 md:h-16 border-b border-border/50 flex items-center px-3 md:px-4 gap-3 md:gap-4 bg-background/80 backdrop-blur-sm flex-shrink-0">
          <Button variant="ghost" size="icon" onClick={() => setSidebarOpen(!sidebarOpen)} className="text-foreground/70">
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {currentChat?.messages.length === 0 ? <div className="h-full flex flex-col items-start md:items-center justify-center p-4 md:p-6">
              ...
              {currentChat?.messages.map(message => <div key={message.id} className={`flex gap-2 md:gap-4 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  {message.role === "assistant"}
                  <div className={`max-w-[85%] md:max-w-[70%] rounded-2xl px-3 md:px-4 py-2 md:py-3 ${message.role === "user" ? "bg-accent text-accent-foreground" : "bg-secondary/50 text-foreground"}`}>
                    <p className="text-xs md:text-sm leading-relaxed whitespace-pre-wrap">{message.content}</p>
                    {message.role === "assistant" && isReport(message.content) && <Button variant="ghost" size="sm" onClick={() => downloadAsFile(message.content, `LAM13-Report-${new Date(message.timestamp).toISOString().split('T')[0]}.md`)} className="mt-2 md:mt-3 gap-2 text-accent hover:text-accent hover:bg-accent/10 h-7 md:h-8 px-2 md:px-3 text-xs md:text-sm">
                        <Download className="w-3.5 h-3.5 md:w-4 md:h-4" />
                        Download Report
                      </Button>}
                  </div>
                  {message.role === "user" && <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-foreground/10 flex items-center justify-center flex-shrink-0">
                      <User className="w-3.5 h-3.5 md:w-4 md:h-4 text-foreground/70" />
                    </div>}
                </div>)}
              {isTyping && <div className="flex gap-2 md:gap-4">
                  ...
                  <div className="bg-secondary/50 rounded-2xl px-3 md:px-4 py-2 md:py-3">
                    <div className="flex gap-1">
                      ...
                      <span className="w-2 h-2 bg-accent/60 rounded-full animate-bounce" style={{
                  animationDelay: "0.1s"
                }} />
                      <span className="w-2 h-2 bg-accent/60 rounded-full animate-bounce" style={{
                  animationDelay: "0.2s"
        ...
        {/* Input Area */}
        <div className="border-t border-border/50 py-3 px-4 md:p-4 bg-background backdrop-blur-sm flex-shrink-0 sticky bottom-0">
          <div className="max-w-4xl mx-auto">
            <div className="flex gap-2 md:gap-3 items-end">
              ...
              }} placeholder="Ask about public sector strategy..." className="w-full resize-none rounded-xl border border-border/50 bg-secondary/30 px-3 md:px-4 py-2.5 md:py-3 text-sm md:text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 min-h-[44px] md:min-h-[52px] max-h-32 md:max-h-40" rows={1} />
              </div>
              <Button onClick={handleSend} disabled={!input.trim() || isTyping} className="bg-accent hover:bg-accent/90 text-accent-foreground h-[44px] w-[44px] md:h-[52px] md:w-[52px] rounded-xl flex-shrink-0">
                <Send className="w-4 h-4 md:w-5 md:h-5" />
              </Button>
            </div>
            {!loggedInUser && <p className="text-[10px] md:text-xs text-muted-foreground text-center mt-2 md:mt-3">
                This is a Beta version. Sign in to save conversations and unlock full capabilities.
              </p>}
          </div>
        </div>
      </main>
